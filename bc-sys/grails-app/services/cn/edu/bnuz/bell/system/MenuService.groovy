...
class MenuService {
    ...
    def getMenus(String root, List<String> roles) {
        def menus = []
        def stack = []
        def level = 1
        def map = [:]
        ...
        getMenusByRoot(root).each {
            def item = [id: it.id, labelCn: it.labelCn, labelEn: it.labelEn, items: []]
            if(it.menuLevel == 1) {
                menus << item
            } else {
        ...
        getMenuItems(root, roles).each {
            map[it.menuId].items << [id: it.id, labelCn: it.labelCn, labelEn: it.labelEn, url: it.url]
        }
